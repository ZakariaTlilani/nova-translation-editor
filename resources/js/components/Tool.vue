  ...
  <div>
    ...
    <div class="flex border-b-2 border-50">
      ...
      <div class="w-4/5 py-2 px-8 relative">
        ...
        <input type="text" :placeholder="__('Filter') + ': By key or translation'"
    ...
    <Card v-if="showTable" class="my-6 rounded-md mb-8">
      ...
      <template v-for="(translation, group) in filterdTranslations">
        ...
        <div v-if="currentGroup === group" :key="group + 'tab'" class="overflow-x-auto">
          ...
          <div class="flex flex-col md:flex-row justify-center space-y-2 md:space-y-0 space-x-3 mb-2">
            <LoadingButton dusk="create-button" type="button"
              ...
              @click="showNewModal = true" :loading="false">
              {{ __('Add new row') }}
            </LoadingButton>

            <LoadingButton dusk="create-button" type="button"
              class="shrink-0 h-9 px-4 focus:outline-none ring-primary-200 dark:ring-gray-600 focus:ring text-white dark:text-gray-800 inline-flex items-center font-bold shadow rounded focus:outline-none ring-primary-200 dark:ring-gray-600 focus:ring bg-primary-500 hover:bg-primary-400 active:bg-primary-600 text-white dark:text-gray-800 inline-flex items-center font-bold px-4 h-9 text-sm shrink-0 h-9 px-4 focus:outline-none ring-primary-200 dark:ring-gray-600 focus:ring text-white dark:text-gray-800 inline-flex items-center font-bold"
              :loading="!loaded" @click="save(currentGroup)">{{
                __('Save') }}
              "{{ currentGroup }}"
            </LoadingButton>
          </div>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-center md:justify-end space-y-2 md:space-y-0 space-x-3"
      ...
      <LoadingButton dusk="create-all-button" type="button"
        class="shrink-0 h-9 px-4 focus:outline-none ring-primary-200 dark:ring-gray-600 focus:ring text-white dark:text-gray-800 inline-flex items-center font-bold shadow rounded focus:outline-none ring-primary-200 dark:ring-gray-600 focus:ring bg-primary-500 hover:bg-primary-400 active:bg-primary-600 text-white dark:text-gray-800 inline-flex items-center font-bold px-4 h-9 text-sm shrink-0 h-9 px-4 focus:outline-none ring-primary-200 dark:ring-gray-600 focus:ring text-white dark:text-gray-800 inline-flex items-center font-bold"
        @click="save" :loading="!loaded">
        {{ __('Save all') }}
      </LoadingButton>
    </div>

    <add-row-modal :group="currentGroup" :existing-keys="existingKeys" v-if="showNewModal" @close="showNewModal = false"
      @createCancelled="showNewModal = false" @create="addRow" />
